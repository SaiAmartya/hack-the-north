...
For any user-facing UI or visual work, read and follow [DESIGN_SYSTEMS.md](DESIGN_SYSTEMS.md). The player UI is game-only: one instruction/action at a time, simple iPhone/badge connection, warm **Wizarding Workshop** setup and dark video-first **Enchanted Mirror** duels. No diagnostic dashboards, implementation notices, replay controls or transport terminology in the player experience. QA runs through scripts and explicitly gated test routes, not game navigation. Keep technical evidence and pending hardware qualification in test reports/docs. Do not introduce external fonts, network assets or copied third-party/franchise visual assets.
