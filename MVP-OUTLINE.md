...
**Team interface:** [BADGE-FIRMWARE-CONTRACT.md](BADGE-FIRMWARE-CONTRACT.md) defines the required badge behavior and v1 bytes. The current checkpoint incorporates teammate main `76b6388`; badge and physical iPhone are player options; badge firmware 0.2.0 is the first gameplay-profile image and is awaiting flash and physical QA. Raw synthetic replay remains script-only QA, never a player control. Both teams must acknowledge interface changes. Start with [teammate installation/run instructions](docs/TEAM-SETUP.md) and [current input evidence](docs/qa/input-rebuild.md); historical firmware results do not qualify a newer image.
...
**Execution plan:** [IMPLEMENTATION-PLAN.md](IMPLEMENTATION-PLAN.md) is the approved complete 0→1 architecture and gate checklist. Keep volatile implementation status, commands, test counts and open physical gates in [current platform evidence](docs/qa/game-platform.md), with badge-specific evidence in the [firmware integration review](docs/qa/firmware-integration-review.md). The architecture below is the target, not a substitute for those reports.
...
2. **Calibrate.** The browser asks for a still grip, then three examples of the jab. Rehearse the incantation. Optional spells (shake, pull back, flick up, slash, key turn, circle) can be learned afterwards or skipped; each new stroke must keep clear of every stroke already learned. Calibration and coaching happen on the laptop; no controller menu navigation is required during play.
...
4. **Duel.** Say “Stupefy!” while making a short jab. Say “Protego!” while shaking the wand side to side. Say “Expelliarmus!” while pulling the wand back sharply, “Incendio!” with an upward flick, “Sectumsempra!” with a sideways slash, “Petrificus Totalus!” while turning the wand like a key and back, or “Expecto Patronum!” while drawing one full circle. Nothing needs to be equipped first.
...
| **Protego:** say the name + shake the wand side to side (three quick wiggles) | Shield lasts 1.2 seconds, blocks one incoming spell, then breaks | 3 s | Cyan curved shield with an impact ripple |
...
| **Petrificus Totalus:** say the name + turn the wand a quarter turn like a key, then back | No damage; body-bind for 1.5 seconds: the target cannot cast anything, not even Protego; 2.4-second travel | 10 s | Pale-blue bolt and a rigid status rune |
...
**Demo network:** venue Wi-Fi is the current constraint. Each laptop frontend may stay at `http://127.0.0.1:5173`, including hosted-phone play. Only the phone controller/pairing service is public; motion prefers direct data-only WebRTC with explicit Internet relay fallback. Game proxies still reach one private referee on laptop A, speech stays local to each laptop and opponent video has a separate WebRTC connection. No public referee or TURN deployment is included. Qualify venue client-isolation behavior rather than assuming direct peers work.
...
The repository is changing quickly. Keep the exact implemented boundary, current commands, automated results and open physical gates in [current platform evidence](docs/qa/game-platform.md); keep firmware-specific review and hardware evidence in the [firmware integration report](docs/qa/firmware-integration-review.md). The architecture and acceptance criteria here remain authoritative even when implementation order changes. A passing replay never establishes physical iPhone, badge, microphone, camera, Windows or human-play evidence.
...
Badge H0–H5 qualification runs alongside the platform gates: it creates badge-specific calibration and qualifies recovery, actual sensor/profile, Windows GATT, combined load, two-badge endurance and real feedback without changing gameplay. iPhone qualification separately measures Safari cadence, observation age, pairing/recovery and HTTPS/WSS behavior. Neither physical source's results stand in for the other. Use [current platform evidence](docs/qa/game-platform.md) and the [firmware integration report](docs/qa/firmware-integration-review.md) for current completion status rather than inferring it from this gate definition.
