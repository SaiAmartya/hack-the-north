...
**Team interface:** [BADGE-FIRMWARE-CONTRACT.md](BADGE-FIRMWARE-CONTRACT.md) defines the required badge behavior and v1 bytes. The current checkpoint incorporates teammate main `76b6388`; badge and physical iPhone are player options, but the current 0.1.9 badge source is diagnostic-only, not a playable release. Raw synthetic replay remains script-only QA, never a player control. Both teams must acknowledge interface changes. Start with [teammate installation/run instructions](docs/TEAM-SETUP.md) and [current input evidence](docs/qa/input-rebuild.md); historical firmware results do not qualify a newer image.
