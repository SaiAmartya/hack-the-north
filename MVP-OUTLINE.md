...
**Selected build (latest user update):** each player chooses **a custom-firmware BLE badge or an iPhone motion controller**, with all recognition/voice/video on their laptop. Badge motion connects directly to Windows Chrome; iPhone Safari uses the approved phone-only public HTTPS/WSS service (optional trusted private-LAN profile). Both feed the same raw-data, calibration and fusion pipeline, local `faster-whisper`, Python referee and Three.js/WebRTC game. The badge remains the prize-track hardware; phone play does not qualify badge hardware. Following the exact-upstream test, Sai authorized a fresh procedural firmware repair and reflash of the connected badge. The other badges remain spares, not receivers.

**Team interface:** [BADGE-FIRMWARE-CONTRACT.md](BADGE-FIRMWARE-CONTRACT.md) defines the required badge behavior and v1 bytes. The current checkpoint incorporates teammate main `76b6388`; badge and physical iPhone are player options; badge firmware 0.2.0 was the first gameplay-profile image and 0.2.1, the current source, is flashed on WAND-B602 with physical QA pending. Raw synthetic replay remains script-only QA, never a player control. Both teams must acknowledge interface changes. Start with [teammate installation/run instructions](docs/TEAM-SETUP.md) and [current input evidence](docs/qa/input-rebuild.md); historical firmware results do not qualify a newer image.

**Phone input rebuild (current decision):** public HTTPS QR + matching-number approval remain, using the personal Cloudflare account only. Prefer a separate data-only, host-ICE WebRTC connection between Safari and the laptop; no STUN/TURN is added. After a ten-second direct preflight failure, offer **Use internet connection** explicitly. A shared versioned phone session carries unchanged 20-byte badge records through either direct RTC or bounded WSS relay. Healthy direct motion survives cloud-signalling loss. Temporary failures retain approved pairing for at most 30 seconds; fresh input generations, endpoint reset/OPEN/SYNC and neutral recheck prevent old casts/cues. Never switch routes or resume an aborted round automatically. Neither route is physically qualified until measured on the venue network. Speech/referee/development routes remain private. Current evidence: [input rebuild](docs/qa/input-rebuild.md).

**Calibration and sensing:** Connect → microphone → illustrated comfortable grip (sideways/slightly diagonal recommended; any consistent grip can be calibrated) → explicit Start calibration → visible three-second stillness → three jabs → three guards → spoken practice → Ready. Never calibrate invisibly during microphone setup. Show Sensor active, Reaching laptop, current coaching and one actionable retry reason; Reset grip does not disconnect. Use timestamp-weighted acceleration-only windows and a continuous trained direction, not per-sample differences or fixed dominant device axes. Screen rotation neither remaps sensor axes nor disconnects the controller. Return to the calibrated grip between movements; use Reset grip when intentionally changing it. Initial thresholds are measured candidates, not frozen physical accuracy claims.

**Badge profile amendment:** verified native 50 Hz/±8 g is preferred; a qualified native 50 Hz/±2 g release is explicitly permitted with truthful INFO, correct conversion/clipping and new calibration. Diagnostic images remain incapable of Ready. The unresolved STATUS overwrite interpretation must be separated from measured acquisition loss; follow the controlled matrix and release gates in the firmware contract.

**Sai's workspace build/QA skill:** [$wand-dev-workflow](.agents/skills/wand-dev-workflow/SKILL.md) guides script-only replay, physical iPhone Safari motion testing over trusted private-LAN HTTPS/WSS and real BLE badge qualification. Its [operating procedure](.agents/skills/wand-dev-workflow/references/workflow.md) includes short human QA cards and explicit timing qualification. Android is not required. Badge and iPhone are both physical player inputs; neither changes the badge wire contract.

**Execution plan:** [IMPLEMENTATION-PLAN.md](IMPLEMENTATION-PLAN.md) is the approved complete 0→1 architecture and gate checklist. Keep volatile implementation status, commands, test counts and open physical gates in [current input evidence](docs/qa/input-rebuild.md), with badge-specific evidence in the [firmware 0.2.x change record](docs/qa/firmware-0.2.0.md). The architecture below is the target, not a substitute for those reports.
...
The physical wand is not a gamepad. **Every spell requires both a spoken incantation and a deliberate movement from the player's selected badge or iPhone.** The spoken name identifies the spell; the motion verifies that the player performed it. No spell selection, button arming, button release, or instant shield button.

Each player chooses one physical source for the room: a battery-powered custom-firmware badge streaming over BLE, or an iPhone streaming measured motion through the paired HTTPS/WSS controller path. The laptop hears the incantation, combines it with movement, shows the opponent, and renders magic. One match server decides combat results. A badge receives compact confirmed physical feedback; an iPhone shows the corresponding bounded controller feedback. No cable or second badge is required during play.

This is **a physical motion wand plus a computer companion**, not the entire game running inside either controller. Custom badge firmware removes the stock Lua sandbox; it does not add a microphone, gyro or graphics processor. The iPhone contributes acceleration only, not phone audio or gameplay authority. Keep speech, classification, multiplayer and rendering on the computers.

### What we ship

| Required | Scope |
| --- | --- |
| Physical wand input | Each player chooses a custom-firmware BLE badge or a paired iPhone motion controller; no gameplay buttons |
...
| Controller experience | Badge: immediate motion LEDs/display and confirmed feedback. iPhone: minimal identity, connection and decoded confirmed-feedback view |
| Setup | Browser-led connection, short calibration, practice and Ready/Rematch controls |

**First playable milestone:** Stupefy versus Protego with real speech and physical badge or iPhone motion on two laptops. **Prize-demo target:** repeat that loop with two qualified badges. **Target completed MVP:** add Expelliarmus and polish those three spells. If only two spells pass the tests, present an explicitly scoped two-spell duel rather than weakening the input contract.
...
Remove mana, extra spells, ultimate attacks, beam clashes, perfect-parry rules, loadouts, chaos/referee badge gameplay and progression. The selected physical controller senses motion and presents bounded feedback; it never decides spell validity or match results. The badge adds prize-track LEDs/display feedback without changing that authority. Focus on one convincing loop: **say it, move the wand, see the magic, counter the opponent**.

## 2. Player journey: no gamepad controls

1. **Connect.** Choose Badge or iPhone on the laptop. For a badge, power it on, click Connect Wand and choose the Bluetooth device matching its on-screen short ID. For an iPhone, open the trusted controller page, enter the short-lived pairing code and enable motion from the required user gesture. Confirm either source with a movement, then grant laptop microphone/camera access. Show only actionable wand, microphone, video and server status; never silently select the first nearby device.
2. **Calibrate.** The browser asks for a still grip, then three examples of the jab and three of the raised guard. Rehearse both incantations. Optional spells (pull back, flick up, slash, chop, circle) can be learned afterwards or skipped; each new stroke must keep clear of every stroke already learned. Calibration and coaching happen on the laptop; no controller menu navigation is required during play.
...
4. **Duel.** Say “Stupefy!” while making a short jab. Say “Protego!” while raising the selected wand into a guard. Say “Expelliarmus!” while pulling the wand back sharply, “Incendio!” with an upward flick, “Sectumsempra!” with a sideways slash, “Petrificus Totalus!” with a downward chop, or “Expecto Patronum!” while drawing one full circle. Nothing needs to be equipped first.
5. **Recover.** Let the controller settle briefly between movements. That natural reset separates casts; there is no press-to-arm action.
6. **Finish.** Both screens show the same result and a simple landed-spells/blocks recap. Click Rematch on the laptop.

**Controller buttons:** no combat bindings on badge or phone. There is no stock launcher/Home behavior to preserve in replacement firmware. The firmware team may retain a documented maintenance/recovery action; Ready, recalibration and rematch remain laptop controls. Deterministic replay is available only through scripts or explicitly gated QA routes and is prohibited in player rooms.

Use a secure grip and small wrist/forearm movements. A badge uses its supplied two AA alkaline batteries; USB is for approved flashing/diagnostics, not gameplay. Keep an iPhone unlocked, foregrounded and firmly held—never throw either device, strike objects, attach the phone precariously to a prop or swing the badge by its lanyard. Follow the official badge power/USB instructions and stop if a device becomes hot or behaves unexpectedly. Custom firmware must preserve safe peripheral/power behavior. [Badge manual](https://badge.hackthenorth.com/manual), [badge safety rules](https://badge.hackthenorth.com/rules)

## 3. Seven spells and a small combat engine
...
Every player learns **Stupefy** and **Protego**; the other five are optional and skippable per player (a player duels with the spells they learned). Each movement is a single sharp stroke in a learned direction (jab, pull, flick, slash), a shake (five or more quick reversals along one line), a twist (the wand rolled like a key and back, with no snap) or one full circle; the four strokes sit on three axes, so every pair is at least 90 degrees apart and the recognizer refuses anything halfway between two of them. Real recordings showed a raise-and-hold guard and a chop both collapsing onto the flick, which is why those two moves became the shake and the twist.

| Spell and required input | Initial effect | Cooldown | Visual |
| --- | --- | --- | --- |
| **Stupefy:** say the name + short jab forward and settle | 20 damage; 2-second projectile travel | 2 s | Crimson bolt with a spark trail |
| **Protego:** say the name + raise the selected wand into a brief held guard | Shield lasts 1.2 seconds, blocks one incoming spell, then breaks | 3 s | Cyan curved shield with an impact ripple |
| **Expelliarmus:** say the name + sharp pull back toward the shoulder | 10 damage and a 1-second disarm (no offensive casts) if unshielded; 2.2-second travel | 6 s | Red-gold ribbon and a broken-wand status rune |
| **Incendio:** say the name + upward flick that returns to the grip | 8 damage on impact, then burns 4 damage per second for 3 seconds; a later shield does not put the fire out; 2-second travel | 6 s | Orange bolt with an ember trail |
| **Sectumsempra:** say the name + sideways slash | 35 damage; 2.8-second travel so it can be answered | 9 s | Steel-white slash |
| **Petrificus Totalus:** say the name + chop straight down and return | No damage; body-bind for 1.5 seconds: the target cannot cast anything, not even Protego; 2.4-second travel | 10 s | Pale-blue bolt and a rigid status rune |
| **Expecto Patronum:** say the name + one full circle | Barrier for 3 seconds that repels every incoming spell without breaking | 15 s | Silver-blue guardian dome |
...
- Every accepted cast starts a short 500 ms global recovery as well as its own spell cooldown; the recovery applies to Protego and Expecto Patronum too. A disarm blocks offensive casts only; a body-bind blocks every cast. Protego and Expecto Patronum still require their own voice, movement and cooldown checks.
...
The motions should be deliberately broad. A stroke (jab, pull, flick, slash, chop) needs an impulse in its learned direction and settling, not accurate distance. Its signed direction is the launch minus the brake along the line that carries the most two-signed energy: the launch is the first lobe carrying a real share of the movement's impulse (recorded flicks, chops and slashes come straight back with a return as strong as the launch, so neither the strongest lobe nor peak velocity is reliable), and the one-signed centripetal pull that a wrist stroke puts along the wand cancels out instead of reading as a jab or pull. Gravity is interpolated from the starting pose to the pose at the end of the movement, so raising into a guard or pulling back to the shoulder does not put a gravity swing on the jab axis. A stroke nearly as strong as its calibrated example beats a matching guard pose; a raise that ends held is the guard, and the fast path waits for the hold whenever the wand is already tilting the way the guard does. Recorded badge strokes run 4–9 g and clip at 8 g; clipped samples are kept. Real recordings (`apps/web/src/input/fixtures/badge-seven-2026-09-19.json`) are the regression set. A guard needs a new raising movement followed by a held orientation; holding the same pose cannot repeatedly recast it. A circle needs the acceleration direction to turn most of the way round in one plane and one sense while the pull stays strong; a stroke's brake turns nothing. If separation is weak, tune those patterns and drop an optional spell before adding a machine-learning project.

### Speech: browser endpointing, local transcription

The laptop microphone runs only during explicit practice/countdown/play after permission and a browser interaction. Request a mono track and `new AudioContext({ sampleRate: 16000 })`; setup verifies the actual context rate is exactly 16 kHz and worklet input is mono, otherwise it fails visibly. An `AudioWorklet` owns the PCM frame timebase and voice endpointing; the initial implementation has no application resampler and does not wait for a controller button. A 2-second quiet calibration establishes an idle-only RMS floor, frozen during speech. Start with 60 ms speech-on hysteresis, 150 ms pre-roll, 200 ms end silence, at most 2.2 seconds of active voice (two-word incantations) and a 3-second hard clip cap. The evidence interval excludes padding.
...
2. Initially allow at most a 350 ms gap between capture intervals, with their combined span no longer than 2 seconds. The final transcript must arrive no later than 1.5 seconds after the voice interval ends (base.en int8 on a laptop CPU needs 500-900 ms per clip whatever its length), in the same audio/ASR generation. A later gesture cannot extend that deadline. Tune these bounds from timestamps and reject ambiguous multiple-motion matches.
...
**Badge transport: custom BLE peripheral → Windows laptop's Bluetooth adapter → Chrome Web Bluetooth.** One badge connection per player, standard GATT reads/notifications/writes, no badge gateway, BLE-native helper or badge Wi-Fi provisioning. The separate loopback speech helper has no badge role. The creator's proven extended-advertising/passive-scan pattern does not qualify connected GATT. Use the contract's [minimal peripheral build](BADGE-FIRMWARE-CONTRACT.md#3-wireless-connection-and-gatt-surface) and [H0–H4 gates](BADGE-FIRMWARE-CONTRACT.md#8-acceptance-and-handoff-checklist): both subscriptions, actual motion and feedback together, measured memory/timing, reconnect and AA power. No scanning, NFC initialization or optional radio roles. A receiver bridge or badge Wi-Fi would add dependencies; neither is built in parallel. Current validation belongs in the evidence reports, not this transport definition. If the minimal connected build fails, return measurements and jointly revise the contract, never silently substitute transport. [Chrome GATT support](https://developer.chrome.com/docs/capabilities/bluetooth), [Espressif peripheral example](https://github.com/espressif/esp-idf/blob/v5.5.3/examples/bluetooth/nimble/bleprph/README.md)
...
The showcase is a spoken Protego plus a raised physical wand—preferably the badge for the prize demo—producing a shield that visibly catches a real opponent's Stupefy. Build that moment before decorative environments. Cap particles and pixel ratio, preload audio, and preserve at least 30 fps on both demo laptops; aim for 60 only after input is stable.

## 7. High-level architecture

```mermaid
flowchart LR
    wandA["Player A: BLE badge or paired iPhone"] <-->|"physical motion and confirmed feedback"| browserA["Browser A: motion, PCM endpointing, fusion, HUD and effects"]
    inputsA["Laptop A microphone and camera"] --> browserA
    browserA <-->|"bounded local PCM/final text"| speechA["Laptop A speech helper :8001"]
    browserA <-->|"Game WebSocket via local Vite"| referee["Python referee: deployed on Render, or laptop A on a LAN"]
    wandB["Player B: BLE badge or paired iPhone"] <-->|"physical motion and confirmed feedback"| browserB["Browser B: motion, PCM endpointing, fusion, HUD and effects"]
...
| Layer | Decision |
| --- | --- |
| Badge firmware — firmware team | Dedicated firmware: fresh acceleration, BLE connection, bounded feedback and diagnostics; no combat authority |
| Frontend — our team | Existing React + TypeScript + Vite; Windows Chrome; Web Bluetooth or paired iPhone motion; AudioWorklet PCM/endpointing; local fusion; plain Three.js |
...
**Demo network:** venue Wi-Fi is the current constraint. Each laptop frontend stays at `http://127.0.0.1:5173`, including hosted-phone play. The phone controller/pairing service and the referee are both public: each laptop's game proxy points at the deployed referee by default (a free Render web service described in [teammate setup](docs/TEAM-SETUP.md#5-multiplayer-over-the-internet-the-default)), or at one private referee on laptop A for a LAN without internet. Players meet in a room addressed by a six-character join code, so one referee serves several duels. Motion prefers direct data-only WebRTC with explicit Internet relay fallback; speech stays local to each laptop; opponent video is a separate WebRTC connection that uses the STUN server, and optionally the Cloudflare TURN credentials, the referee hands both players. Qualify venue client-isolation behavior rather than assuming direct peers work.
...
The earlier Phantom Arena prototype used local radio badges, a gateway, a Python referee and a passive browser viewer; useful infrastructure was reused, not its control model.
...
The repository is changing quickly. Keep the exact implemented boundary, current commands, automated results and open physical gates in [current input evidence](docs/qa/input-rebuild.md) and [teammate setup](docs/TEAM-SETUP.md); keep firmware-specific review and hardware evidence in the [firmware 0.2.x change record](docs/qa/firmware-0.2.0.md). The architecture and acceptance criteria here remain authoritative even when implementation order changes. A passing replay never establishes physical iPhone, badge, microphone, camera, Windows or human-play evidence.
...
| --- | --- | --- |
| **Automated foundation** | Isolated runtime, protocol/transport core, classifier/fusion, referee and script-driven replay behind QA gates | Repeatable positive, negative, fault and two-client flows without exposing replay in player navigation |
| **Input risk gates** | Local ASR plus per-source calibration/classifier/fusion | Exact core words and latency qualify on real laptop microphones; correct badge and iPhone positives/negatives produce no stale attempt |
| **Physical core** | Stupefy/Protego sockets/referee with either Badge or iPhone per player, then video and minimal game-only UI | Two humans complete attack, defense, abort and rematch; source-specific timing/recovery evidence remains labelled |
| **Visual and third-spell gate** | Server-time Three.js effects, performance fallback and conditional Expelliarmus | Defenses, cross-talk, performance, recovery and five-match gates pass without weakening input rules |
| **Windows/demo freeze** | Intended Windows x64 Chrome laptops; qualify iPhone and badge paths separately | Five uninterrupted matches and judge rehearsal on the selected demo source, with H0–H5 retained for badge claims |

Firmware **device/driver bring-up** and **BLE/feedback/recovery** run in parallel with phone, speech, referee and presentation work. H0–H5 remains the badge-specific acceptance stream, not a reason to hide badge play behind an iPhone-only software freeze. Our team owns **device adapter + QA harness**, **recognition/fusion**, **referee/network/video**, and **UI/Three.js**. Mac bring-up can supply early diagnostics, but the intended Windows Chrome/adapters require their own H2–H4 qualification and H5 retains the full two-badge, endurance and gameplay tests. Do not let effects work delay speech or physical-input qualification.
...
## 12. First complete gameplay checkpoint

**A player says “Stupefy” and jabs their selected physical wand; a remote player sees the incoming spell, says “Protego” and raises theirs; the shield blocks before impact, and both computers show the same health. No gameplay button is pressed. Repeat with two badges for the prize-track demo.**
...
For iPhone players, `PhoneSession` sits under `VirtualWandTransport` and reuses the protocol-faithful endpoint in Safari. Its shared phone-only v2 envelope fences generations over direct RTC or explicitly selected WSS, without changing BLE packets. The optional earlier trusted-LAN relay remains a diagnostic route, not the selected rebuilt onboarding. Phone physical qualification cannot qualify BLE, badge sampling or physical badge feedback. Deterministic traces remain separate QA sources and never appear as player choices.
...
**Selected recognizer:** browser-owned PCM timing/endpointing plus a local per-laptop `faster-whisper base.en` CPU-`int8` helper. The browser requests a mono 16 kHz `AudioContext`, verifies the actual rate/input channels, and fails setup if unsupported; there is no initial application resampler. It uses 2 seconds of quiet calibration and the 60 ms start / 150 ms pre-roll / 200 ms end / 2.2-second voice / 3-second clip bounds in Section 4. The helper is English, warm, greedy, temperature zero, no previous-text conditioning, exact canonical vocabulary, one worker and no queue. No Web Speech, cloud ASR, automatic aliases, interim casting or phoneme/fuzzy matching. Qualify both players' Stupefy and Protego before effects; add Expelliarmus only after those work.
...
Each laptop runs its own Vite frontend on the exact default origin `http://127.0.0.1:5173`, opens it in Chrome and runs its own speech helper on fixed loopback `127.0.0.1:8001`. By default both Vite game proxies point at the deployed referee (`https://wandduel-referee.onrender.com`); on a LAN without internet, laptop A runs the referee on its selected private IPv4 port 8000 and both proxies point there instead. Each Vite speech proxy still points to its own loopback helper. Use `/api/game/*` and `/ws/game` for the referee, `/api/speech/{health,transcribe}` for local ASR, and the internally named `/ws/dev-wand` route only for a paired iPhone controller. Both browsers use same-origin paths. Keep Vite loopback-only unless an approved iPhone-input profile is active. [Existing proxy](apps/web/vite.config.ts), [game socket client](apps/web/src/game/client.ts)

**iPhone-input profile:** use the approved phone-only public HTTPS service and direct-first session above. Local Vite remains loopback-only; no phone certificate installation is required. The earlier private-LAN HTTPS profile is optional diagnostics and still requires explicit network/trust approval.

Before LAN use:

- The browser owns BLE and camera; the old host serial gateway, host-camera worker and AI director no longer exist (removed with the Phantom Arena prototype).
...
Use one video-only `RTCPeerConnection` per player, initially 720p at 30 fps with a 480p low preset. Use the perfect-negotiation pattern with an assigned polite role; authenticate and generation-scope signalling, queue ICE until the remote description exists, and mute self-preview. **Vite carries signalling, not video.** Verify direct peer media on the actual network; a working game socket does not prove video connectivity. The referee's welcome message supplies the ICE servers: a public STUN server by default and short-lived Cloudflare TURN credentials when the hosted referee has a TURN key, so video also crosses home and venue NATs.
...
### 13.6 Build and test continuously across QA and physical inputs

**Keep diagnostics out of the player experience.** Scripts and explicitly gated `/__qa/*` routes may inspect identity/boot/session, x/y/z, sample age/loss, calibration, speech/gesture/fusion decisions and decoded feedback acknowledgements. The production setup shows only Badge/iPhone choice, actionable permission/connection/calibration steps and Ready. QA virtual-wand previews are driven by **decoded outbound commands**, not directly by game state, so they still test both sides of the boundary without becoming public controls.
...
| Two browser clients | Automated QA-room duel through virtual raw inputs, real sockets/referee and the minimal game UI; assertions on health/result/effect IDs. Separate browser contexts, synthetic media if needed | Real camera permissions, microphone isolation or Windows BLE |
| Two actual laptops | Real camera/WebRTC/LAN, real microphones and selected physical Badge/iPhone inputs; capture frame rate, network loss/rejoin and speech latency independently | The unselected controller type or badge battery endurance unless separately exercised |

Use [Vitest with fake clocks](https://vitest.dev/guide/mocking/timers) for protocol/classifier/fusion, pytest for the referee and local helper, and [Playwright/Chromium with isolated client contexts](https://playwright.dev/docs/browser-contexts) for repeatable two-client flows. Keep changing commands/counts and exact coverage in [current input evidence](docs/qa/input-rebuild.md). Wand simulation stays browser-native and reachable only from scripts or build-gated QA routes, so Windows needs no PTY or virtual COM driver and players never see replay controls. Automated fixtures cannot replace real microphone plus local-ASR qualification in the actual Chrome installation.
...
**Simulation isolation:** an explicitly enabled QA mode permits replay or mixed fixtures only through scripts and gated test routes; every QA result is permanently labelled. Player rooms expose only physical Badge or iPhone choices and reject synthetic/replay input. Lock the physical source at room creation; switching requires leaving and creating a fresh room, clearing evidence. This prevents accidental demonstration claims, not a malicious client's forged identity. Direct cast injection is allowed only in isolated engine/visual tests, never as the claimed end-to-end test.
...
| --- | --- | --- | --- |
| **G0 — automated foundation** | Isolated duel shell, protocol/virtual/fake BLE, classifier/fusion and QA-only replay | Sai reviews script/browser evidence; no automated result is called physical evidence | Positive/negative/fault flows, feedback expiry, outage/reconnect and source labels pass without replay controls in player UI |
| **G1 — physical input qualification** | Mono 16 kHz local ASR plus shared physical-source calibration/classifier | Sai tests real microphones and each intended Badge/iPhone source | Core words/latency pass; one matching fresh pair produces one attempt; motion/speech/mismatch negatives produce none |
| **G2 — physical playable** | Real decoder/fusion/referee, video and minimal Stupefy/Protego game UI | Two humans complete cards with selected Badge or iPhone inputs; scripted opponent remains QA-only | Attack, defense, abort and rematch pass with measured source timing, nearby-speaker and recovery evidence |
...
| **G4 — Windows/demo freeze** | Run/freeze the complete candidate on two intended Windows x64 Chrome laptops | Two people, local ASR/cameras and the selected physical controllers complete judge rehearsal | Five uninterrupted matches, clean rematches, at least 30 fps and documented source-specific limits; badge claims also require H0–H5 |

Badge H0–H5 qualification runs alongside the platform gates: it creates badge-specific calibration and qualifies recovery, actual sensor/profile, Windows GATT, combined load, two-badge endurance and real feedback without changing gameplay. iPhone qualification separately measures Safari cadence, observation age, pairing/recovery and HTTPS/WSS behavior. Neither physical source's results stand in for the other. Use [current input evidence](docs/qa/input-rebuild.md) and the [firmware 0.2.x change record](docs/qa/firmware-0.2.0.md) for current completion status rather than inferring it from this gate definition.
...
Build the **Stupefy-versus-Protego platform against the contract first**, then qualify both physical input choices without branching gameplay. Do not add a second badge transport, optional-speech gameplay or extra mechanics. The iPhone relay is a physical player-input carrier; deterministic replay remains QA-only. Custom firmware enables a cleaner badge interface but does not eliminate hardware testing.

Current state and the next measured gate live in [docs/qa/input-rebuild.md](docs/qa/input-rebuild.md) and [docs/qa/firmware-0.2.0.md](docs/qa/firmware-0.2.0.md). Continue the approved plan from the first unmet dependency rather than replaying obsolete slice checkpoints. Commit/push, device flashing, TLS/trust setup and publishing retain their separate approval boundaries.
