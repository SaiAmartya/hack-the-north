...
Every player learns **Stupefy** and **Protego**; the other five are optional and skippable per player (a player duels with the spells they learned). Each movement is a single sharp stroke in a learned direction, a raise into a held pose, or one full circle; the five strokes sit on three axes, so every pair is at least 90 degrees apart and the recognizer refuses anything halfway between two of them.
...
The motions should be deliberately broad. A stroke (jab, pull, flick, slash, chop) needs an impulse in its learned direction and settling, not accurate distance. Its signed direction is the launch minus the brake along the line that carries the most two-signed energy: the sign comes from the peak of the integrated velocity, so a wind-up or a brake sharper than the launch cannot flip it, and the one-signed centripetal pull that a wrist stroke puts along the wand cancels out instead of reading as a jab or pull. Gravity is interpolated from the starting pose to the pose at the end of the movement, so raising into a guard or pulling back to the shoulder does not put a gravity swing on the jab axis. A stroke as strong as its calibrated example beats a matching guard pose; a gentle raise that ends held is the guard. A guard needs a new raising movement followed by a held orientation; holding the same pose cannot repeatedly recast it. A circle needs the acceleration direction to turn most of the way round in one plane and one sense while the pull stays strong; a stroke's brake turns nothing. If separation is weak, tune those patterns and drop an optional spell before adding a machine-learning project.
