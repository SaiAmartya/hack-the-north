...
**Calibration and sensing:** Connect → battle lobby → Ready. The microphone starts automatically, Ready requires healthy input, and one grip-agnostic profile serves every player (any firm stroke is Stupefy, any held raise is Protego, a lowering is ignored) with speech still required for every cast. Nothing is calibrated per player in the game: the recognizer re-anchors the resting pose from each still quarter second. Per-player calibration (grip stillness, three jabs, three guards) remains a recognizer capability exercised only by scripted QA. Show Sensor active, Reaching laptop, current coaching and one actionable retry reason; Reset grip does not disconnect. Use timestamp-weighted acceleration-only windows and a continuous trained direction, not per-sample differences or fixed dominant device axes. Screen rotation neither remaps sensor axes nor disconnects the controller. Return to the calibrated grip between movements; use Reset grip when intentionally changing it. Initial thresholds are measured candidates, not frozen physical accuracy claims.
