...
**Demo network:** venue Wi-Fi is the current constraint. Each laptop frontend stays at `http://127.0.0.1:5173`, including hosted-phone play. The phone controller/pairing service is public, and the referee may be too: each laptop's game proxy points either at one private referee on laptop A or at the hosted referee (a free Render web service described in [teammate setup](docs/TEAM-SETUP.md#5-play-over-the-internet-hosted-referee)). Players meet in a room addressed by a six-character join code, so one referee serves several duels. Motion prefers direct data-only WebRTC with explicit Internet relay fallback; speech stays local to each laptop; opponent video is a separate WebRTC connection that uses the STUN server, and optionally the Cloudflare TURN credentials, the referee hands both players. Qualify venue client-isolation behavior rather than assuming direct peers work.
