...
**Selected build (latest user update):** each player chooses **a custom-firmware BLE badge or an iPhone motion controller**, with all recognition/voice/video on their laptop. Badge motion connects directly to Windows Chrome; iPhone Safari uses the approved phone-only public HTTPS/WSS service (optional trusted private-LAN profile). Both feed the same raw-data, calibration and fusion pipeline, local `faster-whisper`, Python referee and Three.js/WebRTC game. The badge remains the prize-track hardware; phone play does not qualify badge hardware. Sai requested discarding local firmware fixes and flashing the teammate's exact upstream source, subject to backup/recovery/power checks. The other badges remain spares, not receivers.

**Team interface:** [BADGE-FIRMWARE-CONTRACT.md](BADGE-FIRMWARE-CONTRACT.md) defines the required badge behavior and v1 bytes. Firmware source is integrated from main `6a50929`; badge and physical iPhone are player options. Raw synthetic replay remains script-only QA, never a player control. Both teams must acknowledge interface changes. See [current firmware review and measurements](docs/qa/firmware-main-6a50929.md); historical 0.1.1/0.1.2 results do not qualify this image.

**Phone onboarding update:** Sai approved a narrowly scoped public HTTPS phone controller/pairing service to remove manual certificate installation, and selected the personal Cloudflare account. The unrelated `at` account stays untouched. Scan a QR, allow phone motion, then approve the matching number on the laptop. Only phone assets and opaque controller traffic go through the public service; never expose Vite, speech or the referee. This is an internet-dependent WSS relay, not direct peer-to-peer Bluetooth. The optional local HTTPS path remains available; protocol bytes, freshness rules, laptop-only speech and badge BLE are unchanged. Deployment and physical qualification status live in [current connection evidence](docs/qa/game-platform.md).
...
**Demo network:** both laptops use a controlled hotspot/LAN. Badge-only play may keep each frontend at `http://127.0.0.1:5173`; iPhone play uses the explicitly approved trusted HTTPS/WSS profile on each selected laptop interface. The two Vite processes proxy game traffic to one session-protected Python referee on laptop A; WebRTC carries video separately. Section 13.4 specifies launch and security boundaries. Public hosting and a TURN relay are outside the initial implementation.
...
For iPhone players, the [workspace workflow skill](.agents/skills/wand-dev-workflow/SKILL.md) uses the internal WSS relay to the same protocol-faithful endpoint core in iPhone Safari. The phone-enabled profile adds trusted LAN HTTPS with explicit setup approval; badge-only and automated development can stay on exact loopback `127.0.0.1`. The phone is a physical player input with its own calibration and measured timing qualification, but it cannot qualify BLE, badge sampling or physical badge feedback. Deterministic traces remain separate QA sources and never appear as player choices.
...
**iPhone-input profile:** the approved profile in the [workflow skill](.agents/skills/wand-dev-workflow/SKILL.md) serves Vite on a selected private interface over trusted HTTPS/WSS. It is required only for iPhone play, does not change the badge BLE topology and does not authorize network or certificate-trust changes by itself.
