...
**Calibration and sensing:** the default is quick play: Connect → Join battle → lobby → Ready (microphone starts automatically; Ready requires healthy input), with a grip-agnostic profile (any firm stroke is Stupefy, any held raise is Protego, a lowering is ignored) and speech still required for every cast. The personal flow remains one click away (Practice first; Join battle can skip it): illustrated comfortable grip (sideways/slightly diagonal recommended; any consistent grip can be calibrated) → explicit Start calibration → visible three-second stillness → three jabs → three guards → spoken practice → Ready. Never calibrate invisibly during microphone setup; quick play does not calibrate at all, it re-anchors the resting pose from each still quarter second. Show Sensor active, Reaching laptop, current coaching and one actionable retry reason; Reset grip does not disconnect. Use timestamp-weighted acceleration-only windows and a continuous trained direction, not per-sample differences or fixed dominant device axes. Screen rotation neither remaps sensor axes nor disconnects the controller. Return to the calibrated grip between movements; use Reset grip when intentionally changing it. Initial thresholds are measured candidates, not frozen physical accuracy claims.
...
2. **Calibrate.** The browser asks for a still grip, then a few gentle examples of the jab, raised guard and sweep. Rehearse each incantation. Calibration and coaching happen on the laptop; no controller menu navigation is required during play.
...
4. **Duel.** Say “Stupefy!” while making a short jab. Say “Protego!” while raising the selected wand into a guard. Say “Expelliarmus!” while sweeping sideways. Nothing needs to be equipped first.
...
| **2 — defense** | **Protego:** say the name + raise the selected wand into a brief held guard | Shield lasts 1.2 seconds, blocks one Stupefy, then breaks; 3-second cooldown | Cyan curved shield with an impact ripple |
