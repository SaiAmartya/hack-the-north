...
test("pooled spell renderer keeps stable resources through 100 cast cycles",async({page})=>{
  ...
  const result=await page.evaluate(async()=>{
    ...
    const after=effects.resourceCounts();effects.dispose();canvas.remove();return {before,after};
  });
  expect(result.after.geometries).toBe(result.before.geometries);expect(result.after.textures).toBe(result.before.textures);expect(result.after.drawCalls).toBeLessThanOrEqual(20);
});
