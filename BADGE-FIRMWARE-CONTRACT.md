...
`effect`: 1 accepted cast, 2 blocked incoming hit, 3 took damage, 4 round result. `spell`: 0 none, 1 Stupefy, 2 Protego, 3 Expelliarmus, 4 Incendio, 5 Episkey. Accepted cast requires spell 1–5; result requires spell 0 and current phase won/lost/draw. Block/damage may include the responsible spell. Duration is 1–1000 ms. The state phase determines the outcome; cues cannot change it. Even within the same epoch, the browser waits for OK on the matching result SET_STATE before sending a result CUE; firmware rejects a result cue while phase is not a result.

**Five-spell extension (September 20, firmware 0.3.0):** IDs 4 and 5 extend the v1 cue vocabulary without changing UUIDs, packet layouts or earlier spell IDs. Firmware before 0.3.0 rejects those two cues; use 0.3.0 for five-spell feedback. Episkey's accepted cue is mint green and its healing arrives in the next authoritative HP state; the badge never applies healing, damage, cooldowns or effects itself.
...
| CUE Episkey | `01 04 04 00 dd cc bb aa 01 05 2c 01 04 03 02 01 78 05 00 00` | Command 4, Episkey accepted-cast cue; same timing/epoch |
