...
**Status:** v1 team handoff, HAL-grounded revision with five-spell cue extension, September 20, 2026. Packet layouts, UUIDs and earlier golden vectors are unchanged. The [0.1.8 controlled matrix](docs/qa/firmware-0.1.8-matrix.md) records partial custom-firmware measurements; the [0.2.x gameplay-profile record](docs/qa/firmware-0.2.0.md) is historical. Current [0.3.0](docs/qa/firmware-0.3.0.md) retains 50 Hz/±8 g and capabilities 0x0F and adds the two new cues. WAND-B602 has a verified app-only flash/readback, 48 passing on-device self-tests and a passing live BLE check; physical speech/gesture play, battery endurance and other badges remain unqualified. Companion: [MVP-OUTLINE.md](MVP-OUTLINE.md).

**In plain English:** deliver a battery-powered wand that connects directly to Chrome, streams trustworthy motion, reports when something is wrong, and displays small feedback messages from the game. We will build all spell recognition, speech handling, multiplayer, combat and Three.js graphics.
...
| Device health, command acknowledgements, safe link-loss behavior | Authoritative Python referee, sessions, rules, HUD and Three.js |
...
For the approved ±2 g alternative, use `[-2000,+2000]` instead and the verified native conversion for that profile. The platform validates against INFO as well as the wire-format ceiling; it never interprets ±2 g readings with a ±8 g conversion. Both ranges reject saturated samples. Capability bits are set only by a firmware profile row the team has marked as the gameplay profile (native 50 Hz/±8 g, starting with image 0.2.0); diagnostic rows keep them zero, and changing that byte solely to remove the setup error is prohibited. Setting the bits does not replace the H-gates: the physical QA card in [the 0.2.0 change record](docs/qa/firmware-0.2.0.md) must be run and recorded before a badge is called qualified.
...
`effect`: 1 accepted cast, 2 blocked incoming hit, 3 took damage, 4 round result. Active `spell` codes are 0 none, 1 Stupefy, 2 Protego, 3 Expelliarmus, 4 Incendio and 8 Episkey. Codes 5–7 are reserved and rejected. Accepted cast requires spell 1, 2, 3, 4 or 8; result requires spell 0 and current phase won/lost/draw. Block/damage may include the responsible spell. Duration is 1–1000 ms. The state phase determines the outcome; cues cannot change it. Even within the same epoch, the browser waits for OK on the matching result SET_STATE before sending a result CUE; firmware rejects a result cue while phase is not a result.

**Five-spell reconciliation (September 20, firmware 0.3.0):** historical firmware 0.2.2/0.2.3 allocated 5 Sectumsempra, 6 Petrificus Totalus and 7 Expecto Patronum. Those IDs remain reserved rather than reused. Episkey uses new ID 8, which the older images reject instead of displaying a different spell; Incendio retains ID 4. Use 0.3.0 for all five cues. UUIDs, 20-byte layouts and earlier spell IDs are unchanged. Episkey's accepted cue is mint green and its healing arrives in the next authoritative HP state; the badge never applies healing, damage, cooldowns or effects itself.
...
STATUS kind 2 remains reserved for the historical 0.2.3 button-cast request and is unsupported by this five-spell game. It is never emitted or interpreted as a cast. Spell recognition still requires physical motion and laptop speech.
...
3. **Synchronize:** perform five sequential SYNC exchanges with distinct command sequences; use the lowest-RTT valid response. For browser send/receive times `t0,t1` and exact device receipt `d`, offset `browser - device` lies within `[t0 - d, t1 - d]`. The wire timestamp is integer milliseconds, not exact fractional receipt time: use the midpoint with `RTT/2 + 1 ms` initial uncertainty to cover relative receipt/sample quantization, then add the drift margin. Record `t0` when actually issuing the write, not when enqueuing it. Do not use late/duplicate/timed-out ACKs as new measurements. This bounds rather than proves one-way delay; accounting for quantization changes no packet format.
4. **Ready:** require supported capabilities, valid baseline, sensor healthy, sync RTT ≤100 ms (network half-RTT ≤50 ms, plus the explicit 1 ms timestamp-quantization allowance), fresh stream and usable microphone/server. Send practice/idle state, register device/boot/input generation with referee and complete grip calibration. No network room token is sent to firmware.
5. **Run:** resync about every 5 seconds after a successful probe; retry failed/out-of-budget probes after 500 ms while the current mapping remains valid. Use a conservative 5 ms/second uncertainty growth budget until measured. If total uncertainty exceeds 100 ms or the last good sync is older than 10 seconds, stop accepting casts and report input unavailable. Repeated failed sync must not silently leave a drifting clock active. Suspend/reset fusion if a mapping change exceeds its previous uncertainty.
6. **Fault/reconnect:** disconnect, boot change, page hide/suspension or input outage invalidates all pending gesture/speech evidence and aborts a live round. Detach old listeners; scope **all** motion/status callbacks to their captured browser connection generation, not just ACKs. Reset received motion sequence/timestamp/buffer state before accepting a new stream; MOTION has no link nonce and can otherwise be confused with a queued callback from the previous connection. A laptop tab switch may retain the GATT connection and its existing CONTROL nonce/next sequence: renew browser input generation/listeners, SYNC, fresh-motion validation, baseline and Ready without sending another OPEN on that live protocol session. An actual carrier reconnect requires a fresh nonce/OPEN as well. No old input replay or automatic round resumption. Firmware returns to advertising on disconnect.
...
| CUE Incendio | `01 04 03 00 dd cc bb aa 01 04 2c 01 04 03 02 01 78 05 00 00` | Command 3, Incendio accepted-cast cue; same timing/epoch |
| CUE Episkey | `01 04 04 00 dd cc bb aa 01 08 2c 01 04 03 02 01 78 05 00 00` | Command 4, Episkey accepted-cast cue; same timing/epoch |
...
| Two-wand soak | 10 minutes, both battery wands and actual laptops at intended 1–3 m spacing, simultaneous motion, local speech and state/cues. No resets, unplanned disconnects or ≥500 ms fresh-input outages; test measured gaps against gesture gates |
