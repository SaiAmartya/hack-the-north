...
**Status:** v1 team handoff, HAL-grounded revision with five-spell cue extension, September 20, 2026. Packet layouts, UUIDs and earlier golden vectors are unchanged. The [0.1.8 controlled matrix](docs/qa/firmware-0.1.8-matrix.md) records partial custom-firmware measurements; the [0.2.x gameplay-profile record](docs/qa/firmware-0.2.0.md) records a prior WAND-B602 flash/readback, not complete hardware qualification. Current source [0.3.0](docs/qa/firmware-0.3.0.md) retains 50 Hz/±8 g and capabilities 0x0F, adds the two new cues and has passed local compilation; its flash and physical QA remain pending. Companion: [MVP-OUTLINE.md](MVP-OUTLINE.md).
