...
**Current direction:** firmware arrived on main `6c1b857`, and Sai requested real-badge integration, physical iPhone play, implementation across non-firmware segments, script-driven QA and a minimal game-only UI. Use [current input evidence](../../../../docs/qa/input-rebuild.md) and the [firmware 0.2.x change record](../../../../docs/qa/firmware-0.2.0.md) for status. Do not equate automated passes, iPhone measurements or teammate-reported BLE tests with local badge acceptance. This procedure authorizes no flashing, certificate trust or network changes.
...
**Selected phone onboarding:** public HTTPS QR + matching-number approval, then a **direct data-only WebRTC** connection (host-only ICE, ten-second preflight). If venue Wi-Fi prevents peers, offer **Use internet connection** explicitly through the bounded WSS relay. No silent route switch, STUN/TURN service or native Swift app. One shared phone session/envelope and the unchanged badge-protocol endpoint serve both carriers. The optional older private-LAN HTTPS path is diagnostic, not the selected rebuild. See [input rebuild evidence](../../../../docs/qa/input-rebuild.md). The initial motion permission requires a tap; no phone audio/camera permission.
...
- QR URLs contain a public rendezvous ID, not a bearer capability. The phone requests motion after a tap, waits for a fresh sample, and displays a challenge; the laptop approves that challenge. Credentials are first-frame only. Temporary failures may retain approval for 30 seconds using in-memory role-specific resume credentials (hashes server-side), bounded by absolute expiry. Explicit leave, reload/lost credentials, source change and expiry require a new pair. Fresh logical links reset the endpoint before INFO/subscriptions/OPEN sequence zero/new nonce/SYNC. Old casts, cues and interrupted examples never survive.
...
- Keep calibration explicit: microphone → illustrated comfortable grip (sideways/slightly diagonal recommended, any consistent grip allowed) → Start calibration → three seconds visible stillness → movement-only examples → spoken practice. Stillness movement resets progress; Reset grip does not disconnect. Completed calibration may resume only for the same device/profile/grip after one second of fresh neutral input. A hidden/locked phone requires a foreground Resume tap and fresh Ready.
- Sensor active is based on fresh finite observations; Reaching laptop is based on accepted-motion receipts, not socket state. Both expire. Coaching/progress/orb update at 10 Hz independent of acquisition. Optional details and an explicit last-minute sanitized trace export are essential troubleshooting affordances, not an exemption to log audio/transcripts/tokens/addresses/device IDs.
- Script qualification must exercise moving traces, visible calibration and practice; a constant +1g ten-minute stream proves only transport. Run direct and explicit relay separately, then real Safari movement, held-out recognition and real speech. Preserve 150 ms gap, 200 ms age/stall and 500 ms input-abort limits.
...
These are durable run steps. Use [teammate setup](../../../../docs/TEAM-SETUP.md) for the exact launch commands and [current input evidence](../../../../docs/qa/input-rebuild.md) for implemented controls and known gaps rather than copying stale commands from this procedure.

1. **Ask for the setup check.** The agent confirms the feature branch, clean/known diff, versions, free ports, iPhone/iOS version and controlled network. No legacy serial reader, host camera or AI director exists any more; the launcher runs only the referee, speech helper and frontend. Replay-only work stays loopback-only; live iPhone testing requires the explicitly approved LAN profile below.
...
6. **Calibrate and practice.** Keep Safari foregrounded, the phone unlocked and held safely in a consistent comfortable grip; sideways/slightly diagonal is recommended, not required. Calibrate in the laptop app, keep its microphone near you, then say Stupefy and perform a short jab. Confirm one exact local-ASR final plus one matching motion became one fused attempt, not merely a motion pulse. The phone shows only minimal connection/motion health and decoded feedback; deeper diagnostics stay in scripts or a gated QA build.
...
Verify the six gravity directions; do not assume that browser/native axes, screen orientation and the user's grip are identical. Preserve fixed device coordinates: screen-layout rotation does not remap axes, pause sensing or discard calibration. Recognition learns gravity and movement directions from any consistent grip. Stable wrong-grip input cannot re-arm; use Reset grip when intentionally changing the starting grip. An accelerometer cannot distinguish re-gripping from an identical intentional guard, so do not promise automatic intent detection. Null/non-finite values mean unavailable input, not zero motion. Browser motion permission, cadence and background behavior depend on the device/browser; a click-triggered permission request is required where that API is present. [Motion specification](https://www.w3.org/TR/orientation-event/#deviceorientation), [permission API](https://developer.mozilla.org/en-US/docs/Web/API/DeviceMotionEvent/requestPermission_static)
...
**Current checkpoint:** the full implementation plan is approved and is being executed across the platform and both physical-input paths. Read [current input evidence](../../../../docs/qa/input-rebuild.md) for implemented surfaces, exact checks and the next unmet gate; read the [firmware 0.2.x change record](../../../../docs/qa/firmware-0.2.0.md) for badge-specific evidence. Do not repeat obsolete slice cards or turn automated replay into player UI. Certificate trust/network exposure, firmware flashing, commits and remote writes retain their separate approval boundaries.
